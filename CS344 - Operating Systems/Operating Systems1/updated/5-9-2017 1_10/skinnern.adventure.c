...
struct ConstrRoom* scanAndRead(){
	...
	for (i=0; i<7; i++){
		 if ( house[i].name[0] == '\0' )
			sscanf(line,"ROOM NAME: %s",house[i].name);

		if ( house[i].cons[0][0] == '\0' )
			sscanf(line,"CONNECTION 0: %s",house[i].cons[0]);

		if ( house[i].cons[1][0] == '\0' )
			sscanf(line,"CONNECTION 1: %s",house[i].cons[1]);

		if ( house[i].cons[2][0] == '\0' )
			sscanf(line,"CONNECTION 2: %s",house[i].cons[2]);

		if ( house[i].cons[3][0] == '\0' )
			sscanf(line,"CONNECTION 3: %s",house[i].cons[3]);

		if ( house[i].cons[4][0] == '\0' )
			sscanf(line,"CONNECTION 4: %s",house[i].cons[4]);

		if ( house[i].cons[5][0] == '\0' )
			sscanf(line,"CONNECTION 5: %s",house[i].cons[5]);	

		if ( house[i].cons[6][0] == '\0' )
			sscanf(line,"CONNECTION 5: %s",house[i].cons[6]);

		if ( house[i].type[0] == '\0' )
			sscanf(line,"ROOM TYPE: %s",house[i].type);
	}
	return house;
