...
Display( )
{
	...
	*/
	...
	for (int i = 0; i < 68; i++)
	{
		...
		{
			Xp = Xarr[i][j] + TransXYZ[0];
			Yp = Yarr[i][j] + TransXYZ[1];
			...
			else
			{
				if (j >= 1)
				{
					DrawHyperbolicLine(Xarr[i][j - 1] + TransXYZ[0], Yarr[i][j - 1] + TransXYZ[1], Xarr[i][j] + TransXYZ[0], Yarr[i][j] + TransXYZ[1]);
					/*
					for (int n = 0; n < 100; n++)
					{
					float t = n / 100.;
...
InitGraphics( )
{
	...
	if (fp == NULL)
	{
		fprintf(stderr, "Cannot open 'proj04.dat' !\n");
		exit(1);
	}
	...
	for (int i = 0; i < 68; i++)
	{
		...
		for (int j = 0; j < Npts[i]; j++)
		{
			fscanf(fp, "%f %f", &Xarr[i][j], &Yarr[i][j]);
		}
	}
	fclose(fp);
